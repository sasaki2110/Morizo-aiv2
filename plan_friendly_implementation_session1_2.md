...
セッション1で実装したヘルプ機能のテスト計画です。

**テストの目標**: バックエンドの基本動作を確認する
...
**テストファイルの場所**: `/app/Morizo-aiv2/tests/makehelp/test_help_integration_session1.py`

**参考ファイル**: 
- `/app/Morizo-aiv2/tests/test_bible_regression_check.py`（統合テストの構造を参考）
- `/app/Morizo-aiv2/tests/archive/rebuild/00_1_test_util.py`（認証ユーティリティを参考）
...
```bash
# テスト実行（サーバーが起動していることを確認）
cd /app/Morizo-aiv2
python tests/makehelp/test_help_integration_session1.py
```

### テストファイル作成の注意事項

1. **セッション状態の検証**: セッション状態はHTTP API経由では直接取得できないため、応答内容から間接的に確認するか、セッション状態取得用のエンドポイントを追加する必要があります。

2. **エラーハンドリング**: 各テストケースで適切なエラーハンドリングを実装します。

3. **タイムアウト設定**: 長時間かかる処理がある場合は、タイムアウトを適切に設定します。

4. **ログ出力**: テスト実行中の詳細なログを出力し、デバッグを容易にします。
