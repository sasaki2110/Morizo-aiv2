...
1. **レベル0（通常モード）**: 通常のチャット機能
2. **レベル1（全体概要）**: 「使い方を教えて」→ 4つの機能を紹介
3. **レベル2（機能別詳細）**: 数字（1-4）入力 → 各機能の詳細説明

### ユーザー体験の考慮点

- **自然な会話**: 技術用語を使わず、フレンドリーな応答
- **誤検知防止**: 「使い方」だけでは検知しない、明確なキーワード判定
- **スムーズな復帰**: 通常のチャット入力で自動的にヘルプモードを終了
- **段階的な情報提示**: 一度に全てを表示せず、必要な部分だけを表示
...
**修正の理由**: 
- `plan_friendly.md`の内容をそのまま実装に反映
- フレンドリーな応答文を生成
- コードの可読性と保守性を向上
...
3. **通常処理との競合**: ヘルプモード中でも、通常のチャット入力で通常処理に進めるようにする

4. **plan_friendly.mdとの整合性**: 実装内容は`plan_friendly.md`の内容と完全に一致させる

5. **ログ出力**: ヘルプモードの開始・終了・状態遷移をログに記録する
